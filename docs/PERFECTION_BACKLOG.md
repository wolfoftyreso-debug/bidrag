...
- M13 Delad rate-limit-store (red team RT03-adversariell) — `@fastify/rate-limit` använder in-memory-store; i Vercels serverless-modell delas inte per-IP-räknarna mellan instanser (LIMITATIONS §12). Wire:a en delad store (Vercel KV / Upstash Redis) så gränserna håller vid horisontell skalning. Kräver infra/plan-beslut.
- M14 SSRF DNS-rebinding-residual (red team RT03-adversariell) — källhämtningen revaliderar varje redirect-hop, men `assertSafeUrl` och `fetch` gör separata DNS-uppslag (LIMITATIONS §13). Full stängning kräver att man kopplar upp mot en pinnad IP med explicit Host-header. Låg exponering (källregistrering kräver icke-självbetjänings-rollen data_curator).
