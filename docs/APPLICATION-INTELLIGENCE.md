...
| §3 Källhierarki | Delvis | Källa + kvalitetsgrad per regelverk; källfärskhet vaktas i granskningen (passerat omverifieringsdatum ⇒ flagga). Full hierarki + CONFLICT-detektion återstår |
...
| §6 Requirement Matrix | Delvis | Schemamotor med kanoniska nycklar och villkorsfält; 5/55 stöd har kurerat schema (Kulturrådet, Erasmus+, försörjningsstöd, Nordisk kulturfond, MUCF) — stöd utan schema flaggas öppet i granskningen |
...
| §10–11 Evidence engine E0–E4 + claim register | **Implementerad (E0–E2)** | E0 obesvarat, E1 eget svar, E2 styrkt av bifogat dokument enligt KURERAD kriterium↔bilaga-koppling (`CriterionDef.evidenceKinds`) — utan koppling stannar nivån på E1, aldrig gissning. E3/E4 (externa/verifierade källor) återstår. Claimregister: sifferkonflikter via konsistensmotorn |
