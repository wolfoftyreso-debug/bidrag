...
Samtliga 96 operationer, grupperade. Webbappen använder exakt dessa ytor —
...
| `GET /v1/admin/opportunities` | Granskningskön: stöden i granskningsordning — förfallna först, sedan efter visningar för riktiga användare senaste 30 d — med kureringsstatus, senaste protokoll (vem/när/anteckning) och flaggan "startsida som källa". |
...
| `POST /v1/admin/opportunities/:id/source-check` | Källkontroll i granskningsögonblicket: hämtar stödets källa nu (SSRF-säkert) och svarar med HTTP-status, förändring sedan senaste snapshot och sammanfattning — eller ärligt att ingen källa är registrerad. |
| `POST /v1/admin/opportunities/:id/verify` | Lyft till human_verified — endast med fullständigt granskningsprotokoll (källan finns kvar, villkor, belopp, ansökningssätt + underlag, stödets egen källsida); ofullständigt protokoll vägras med 400. Sparar protokollet som granskningsärende och i revisionsspåret; källadress/ansökningsadress kan rättas i samma steg. |
...
| `npm run seo:check` | SEO-QA-crawlen: titlar, canonical, JSON-LD, intern länkgraf, orphans och sitemap-täckning för den genererade ytan, plus entitetsgrafens vakt (utgivare, geografi, målgrupp, ärliga datum mot seeden). |
...
| `npm run seo:precheck` | Vakten för klusterhubbarnas behörighetskontroll (CONTENT_ENGINE F0 modul 4/5): frågorna ordagrant ur seeden, cores utfall på tre scenarier, sidorna bär verktyget och bundlen håller viktbudgeten. Körs i verify och CI. |
| `npm run seo:precheck:ui` | Behörighetskontrollen i riktig webbläsare: serverar den genererade ytan lokalt och klickar igenom /bidrag/bostadsbidrag/ (kräver seo:build + Chromium). |
