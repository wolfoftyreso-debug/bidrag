...
step "Motorsimulering: ~11 000 personor × alla stöd mot core — DÖD/LÄCKA fäller (revision 2026-09-01)" node --experimental-strip-types tools/simulate-engine.mjs --n=2000 --strict
if [ "$db_up" = 1 ]; then
...
step "Behörighetskontrollen på klusterhubbarna: frågor ordagrant ur seeden, motorns utfall, sidorna bär verktyget (F0 modul 4/5)" node --experimental-strip-types tools/precheckcheck.mjs
