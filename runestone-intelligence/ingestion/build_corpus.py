...
def build(
    *,
    ...
    match_report = match_images(srd_report.imported, img_report.imported, img_report.pairings)

    split = stone_level_split(
        [ins["inscription_id"] for ins in srd_report.imported], seed=seed,
    )

    inscriptions = srd_report.imported
    matched_ids = {img["image_id"] for img in match_report.matched}
    images = match_report.matched + [
        img for img in img_report.imported if img["image_id"] not in matched_ids
    ]
