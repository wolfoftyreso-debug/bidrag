...
| 1 | **Corpus ingestion** *(motor komplett)* | Rundata/SRD-importer, Runor metadata-importer, K-samsök-connector, image provenance pipeline, dataset versioning → **Runestone Corpus v0.1**. Hela motorn (signum, SRD-/HF-/Runor-importers med spårbar berikning, matcher, split, manifest, coverage-rapport, e2e-CLI) är byggd och testad mot fixturer; skarp körning väntar på Uppsala-rapportering + skarpa API-adaptrar |
